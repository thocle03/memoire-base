...
\textbf{Présenté par :} Thomas CLEMENT\\
...
en cascade à deux niveaux estime de manière instantanée les émissions
globales de \(CO_2\). Les résultats démontrent une précision de
prédiction supérieure à 95 \% (RMSE de 15,5 tonnes de \(CO_2\)) et un
temps d'exécution de 0,2 seconde, éliminant les goulots d'étranglement
computationnels (surcharge RAM/SWAP) et ouvrant la voie à des boucles
d'optimisation hybrides (IA-SUMO) pour les planificateurs de la ville
intelligente.
...
\item
  ...
  Sao Bien. (\emph{Chapitre 7, Section 7.1})
\item
  ...
  Reversal}). (\emph{Chapitre 7, Section 7.3.2})
\item
  \textbf{Figure 4 :} Pipeline de prédiction en cascade à deux niveaux
  (\emph{Two-Stage Cascade Model}) pour l'estimation de la vitesse
  moyenne ajustée et des émissions de \(CO_2\) finales sous intempéries
  régionalisées. (\emph{Chapitre 6, Section 6.4})
\end{itemize}
...
\item
  ...
  constante de Kreiss). (\emph{Chapitre 5, Section 5.7.2})
\item
...
\item
  ...
  (filtrage DOM via sumolib) pour cinq villes types. (\emph{Chapitre 8,
  Section 8.3})
\item
...
carbone (\(CO_2\)) à l'échelle planétaire (City of Paris, 2024).
...
progiciel open-source \textbf{SUMO} (Simulation of Urban MObility)
(Krajzewicz et al., 2012). Ces simulateurs microscopiques permettent de
modéliser le comportement de chaque véhicule individuel seconde par
seconde, en intégrant des lois physiques rigoureuses de poursuite de
véhicules (car-following) et de changement de voie. Cette approche
granulaire offre une précision inégalée pour estimer les temps de
parcours réels, identifier les goulots d'étranglement locaux et
quantifier les émissions instantanées de polluants à partir de banques
de données de facteurs d'émissions calibrées (HBEFA3) (Kratzsch et al.,
2020).
...
au réseau urbain (Boeing, 2017) et en exploitant des concepts issus de
l'analyse non-normale (Trefethen \& Embree, 2005) et de la théorie des
perturbations d'opérateurs (Kato, 1995), nous construisons un moteur
prédictif capable de se substituer au simulateur physique en une
fraction de seconde, offrant un outil d'aide à la décision d'une agilité
inédite pour les planificateurs de la ville intelligente.
...
Que se passe-t-il si l'on modifie un axe (perturbation du graphe
\(\delta A = \epsilon B\)) ? En utilisant la \textbf{théorie des
perturbations de Kato}, nous calculons la \textbf{dérivée première et la
dérivée seconde des valeurs propres} simples \(\lambda_i\) associées aux
vecteurs propres à droite \(v_i\) et à gauche \(w_i\) : 1.
\textbf{Dérivée Première (Sensibilité linéaire)} :
\[\lambda_i^{(1)} = \frac{w_i^T B v_i}{w_i^T v_i}\] 2. \textbf{Dérivée
Seconde (Couplage non-linéaire de second ordre)} :
\[\lambda_i^{(2)} = w_i^T B S_i B v_i\] Où
\(S_i = \lim_{z \to \lambda_i} (zI - A)^{-1}(I - P_i)\) est la
résolvante réduite, avec \(P_i = \frac{v_i w_i^T}{w_i^T v_i}\) le
projecteur spectral.

\textbf{Loi de contrôle :} L'objectif mathématique devient de concevoir
une loi pour optimiser (minimiser/maximiser) ces dérivées afin
d'améliorer la robustesse du graphe sans recalculer le spectre de la
matrice complète :
...
\subsection{Chapitre 6 : Le moteur d'intelligence artificielle prédictif
en
cascade}\label{chapitre-6-le-moteur-dintelligence-artificielle-pruxe9dictif-en-cascade}
...
urbaine'' spectrale et topologique développée au Chapitre 5, ce moteur
réalise des prédictions instantanées d'émissions de \(CO_2\) sous charge
de trafic tout en tenant compte des taux catégoriels d'électrification
des véhicules.
...
\subsubsection{L'Espace Multidimensionnel des Descripteurs : Analyse
Détaillée des 43
Features}\label{lespace-multidimensionnel-des-descripteurs-analyse-duxe9tailluxe9e-des-43-features}

Pour capturer toute la complexité cinématique, la structure topologique
brute, les propriétés spectrales dynamiques, la physique capacitive du
réseau et les particularités comportementales régionales, nous avons
conçu un vecteur de caractéristiques (\(\mathcal{X}\)) composé de
\textbf{43 descripteurs}.

La météo (pluie, neige) a été exclue de ce vecteur d'apprentissage. En
effet, inclure les conditions climatiques comme descripteurs directs
dans le modèle XGBoost introduisait des instabilités et des biais de
prédiction. Nous avons opté pour une modélisation découplée : la météo
est appliquée comme un coefficient multiplicatif analytique sur la
vitesse prédite sur sol sec, ou modélisée par des bases de données
distinctes (scénarios de pluie et de neige purs).
...
\subsubsection{Importance des variables (Feature Importance CO2) et
Performances}\label{importance-des-variables-feature-importance-co2-et-performances}

Le métamodèle en cascade a été entraîné sur la base consolidée des 19
simulations d'échelle réaliste (comprenant les nouvelles morphologies
urbaines de Casablanca, Chamonix, Genève, Londres, Rio de Janeiro et
Sydney). Les performances obtenues en validation croisée pour la
prédiction du \(CO_2\) sont résumées ci-dessous :
...
\bottomrule\noalign{}
\endlastfoot
\textbf{XGBoost (Cascade)} & \textbf{15 506,21} & \textbf{13 343,48} &
\textbf{95,27 \%} \\
...
\tightlist
\item
  \textbf{XGBoost (Cascade)} : Il surclasse largement les autres
  approches avec un \(R^2\) de \(95,27\ %
  \). L'enchaînement à deux niveaux permet de contraindre les
  prédictions d'émissions par la réalité physique de la vitesse, évitant
  les aberrations prédictives sous forte charge de trafic.
\item
  ...
  de congestions géométriques brutales, limitant son score à \(79,56\ %
  \).
\item
...
\paragraph{Classement des variables les plus importantes (Feature
Importance
CO2)}\label{classement-des-variables-les-plus-importantes-feature-importance-co2}
...
lumière la hiérarchie d'influence physique suivante : 1.
\textbf{\texttt{nb\_total\_veh} (46,50 \%)} : Le volume global de
circulation reste le facteur de premier ordre. 2.
\textbf{\texttt{nb\_voitures} (35,32 \%)} : Le nombre de voitures
détermine l'effort thermique de base. 3. \textbf{\texttt{nb\_motos}
(8,25 \%)} : La part de deux-roues motorisés, caractéristique des
environnements asiatiques. 4. \textbf{\texttt{duree\_sim\_s} (6,57 \%)}
: La durée totale d'exposition du réseau à la charge de trafic. 5.
\textbf{\texttt{nb\_camions} (0,96 \%)} : Les camions ont des taux
d'émissions unitaires élevés. 6. \textbf{\texttt{origin\_Europe} (0,69
\%)} : La distinction géographique (les villes européennes radiales
réagissant différemment aux charges que les grilles rectilignes). 7.
\textbf{\texttt{pct\_bus\_ev} (0,42 \%)} : L'électrification des bus est
une variable d'action majeure. 8. \textbf{\texttt{sigma\_max\_weighted}
(0,38 \%)} : La valeur singulière maximale pondérée (physique) s'impose
comme la première métrique spectrale prédictive des surémissions. 9.
\textbf{\texttt{h2\_norm\_weighted} (0,24 \%)} : La capacité
d'accumulation de la congestion du réseau routier.
...
\subsubsection{Annexe A : Tableau des indicateurs macroscopiques des
simulations
complètes}\label{annexe-a-tableau-des-indicateurs-macroscopiques-des-simulations-compluxe8tes}
...
\subsubsection{\texorpdfstring{Annexe B : Tonnage de \(CO_2\) émis par
classe de véhicule de la
flotte}{Annexe B : Tonnage de CO\_2 émis par classe de véhicule de la flotte}}\label{annexe-b-tonnage-de-co_2-uxe9mis-par-classe-de-vuxe9hicule-de-la-flotte}
...
\subsubsection{Annexe C : Profil de performance informatique de
l'exécution (secondes
CPU)}\label{annexe-c-profil-de-performance-informatique-de-lexuxe9cution-secondes-cpu}
...
\subsubsection{Annexe D : Index des illustrations graphiques et des
visualisations de
simulation}\label{annexe-d-index-des-illustrations-graphiques-et-des-visualisations-de-simulation}
...
\paragraph{Annexe D.1 : Planches relatives au hub de recharge de Sao
Bien
(Hanoï)}\label{annexe-d.1-planches-relatives-au-hub-de-recharge-de-sao-bien-hanouxef}
...
\paragraph{Annexe D.2 : Planches de topologie spectrale et mathématique
des réseaux
urbains}\label{annexe-d.2-planches-de-topologie-spectrale-et-mathuxe9matique-des-ruxe9seaux-urbains}
...
\paragraph{Annexe D.3 : Planches d'évaluation du modèle d'Intelligence
Artificielle}\label{annexe-d.3-planches-duxe9valuation-du-moduxe8le-dintelligence-artificielle}
...
\paragraph{Annexe D.4 : Planches de cartographie des congestions et
heatmaps de
simulation}\label{annexe-d.4-planches-de-cartographie-des-congestions-et-heatmaps-de-simulation}
