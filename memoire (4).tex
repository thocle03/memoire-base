...
données de terrain, ainsi qu'à la constitution du jeu de données
multi-villes. Elle décrit le protocole d'observation mis en œuvre à
Vinhomes Ocean Park (Hanoï) pour acquérir la vérité terrain de la
...
systématiques de détection. Enfin, elle formalise la constitution de
notre base d'apprentissage multi-villes à grande échelle issue de
simulations de référence pour l'entraînement de notre IA.
...
simulateur SUMO (équations de poursuite de Krauß, connexité et
algorithme de Tarjan). D'autre part, elle présente le formalisme de
l'analyse spectrale des graphes asymétriques et de la théorie des
perturbations de Kato, avant de détailler la conception de notre
métamodèle d'intelligence artificielle prédictif direct.
...
localisée du hub de recharge de Sao Bien à Hanoï (Chapitre 8),
modélisant les dynamiques de files d'attente et le comportement
stochastique des sessions de recharge. En second lieu, elle déploie un
cadre expérimental global (Chapitre 9) pour tester la généricité de
notre modèle sur six réseaux urbains mondiaux de morphologies
contrastées, comparant les exigences computationnelles (temps CPU,
empreinte mémoire RAM) de la micro-simulation physique face à notre
modèle d'intelligence artificielle spectral.
...
Le corpus d'apprentissage est constitué à partir de \textbf{19
...
\subsubsection{\texorpdfstring{Structuration de la base de données
d'apprentissage
(\(X, y\))}{Structuration de la base de données d'apprentissage (X, y)}}\label{structuration-de-la-base-de-donnuxe9es-dapprentissage-x-y}

Chaque simulation de référence achevée est ensuite traitée pour
alimenter notre matrice d'entraînement :

\begin{itemize}
\tightlist
\item
  \textbf{Extraction de la variable cible (\(y\))} : Le fichier d'output
  de SUMO \texttt{tripinfo.xml} est parsé à la fin de chaque exécution.
  Nous calculons la somme totale des rejets de \(CO_2\) (en kg) émis par
  l'ensemble des véhicules actifs sur la durée de la simulation. C'est
  cette valeur cible absolue que notre modèle XGBoost doit apprendre à
  prédire.
\item
  \textbf{Construction du vecteur de caractéristiques (\(X\))} : Pour
  chaque simulation, nous associons un vecteur de 43 descripteurs. Ces
  descripteurs combinent les métriques de demande (nombre total de
  véhicules injectés, durée), les taux d'électrification, et surtout les
  signatures spectrales de la matrice d'adjacence physique du réseau
  (rayon spectral, constante de Kreiss, etc.) calculées une seule fois
  hors ligne.
\end{itemize}

Le corpus final se présente ainsi sous la forme d'une matrice consolidée
où chaque ligne représente l'état macroscopique stabilisé d'une ville
sous une charge de trafic spécifique, reliant ses propriétés
géométriques et cinématiques à ses émissions environnementales globales.
