...
*   **Figure 1 :** Morphologie géométrique et contraintes d'accès du hub de recharge de Sao Bien (12 bornes en épi à 135 degrés) et de la zone d'attente de taxis GSM le long de la voie de service. (*Chapitre 7, Section 7.1*)
*   **Figure 2 :** Représentation schématique du flux de circulation et du tissage des motocyclistes au niveau de l'insertion sur l'avenue Sao Bien. (*Chapitre 7, Section 7.1*)
*   **Figure 3 :** Répartition modale comparée du fluide de trafic lors d'un midi de semaine classique (*Regular Midday Baseline*) face à l'inversion modale observée en période de congés (*Holiday Reversal*). (*Chapitre 7, Section 7.3.2*)
*   **Figure 4 :** Pipeline de prédiction en cascade à deux niveaux (*Two-Stage Cascade Model*) pour l'estimation de la vitesse moyenne ajustée et des émissions de $CO_2$ finales sous intempéries régionalisées. (*Chapitre 6, Section 6.4*)
...
*   **Tableau 1 :** Caractéristiques topologiques générales des 21 villes étudiées (nombre de nœuds, d'arêtes, densité de connexion, degré moyen, indice d'asymétrie, ratio de sources et de puits). (*Chapitre 5, Section 5.7.1*)
*   **Tableau 2 :** Métriques spectrales de non-normalité des matrices d'adjacence non pondérées pour les 21 réseaux urbains (asymétrie, rayon spectral, valeur singulière maximale, norme $H_2$, constante de Kreiss). (*Chapitre 5, Section 5.7.2*)
*   **Tableau 3 :** Performances comparatives des modèles d'apprentissage machine pour la prédiction des émissions de $CO_2$ (RMSE, MAE, $R^2$ de XGBoost, Ridge, Random Forest et MLP). (*Chapitre 6, Section 6.5*)
*   **Tableau 4 :** Profil d'importance des variables (*Feature Importance*) dans le modèle final de prédiction de $CO_2$ par XGBoost. (*Chapitre 6, Section 6.5*)
*   **Tableau 5 :** Taille géométrique des fichiers net.xml sur disque et occupation correspondante de la mémoire RAM en Python (filtrage DOM via sumolib) pour cinq villes types. (*Chapitre 8, Section 8.3*)
...
## Chapitre 6 : Le moteur d'intelligence artificielle prédictif en cascade

Pour s'affranchir définitivement du coût computationnel prohibitif des simulateurs physiques multi-agents comme SUMO, ce chapitre présente la conception, la formulation et la validation expérimentale de notre métamodèle d'intelligence artificielle. En s'appuyant sur la "signature urbaine" spectrale et topologique développée au Chapitre 5, ce moteur réalise des prédictions instantanées d'émissions de $CO_2$ sous charge de trafic tout en tenant compte des taux catégoriels d'électrification des véhicules.
...
### L'Espace Multidimensionnel des Descripteurs : Analyse Détaillée des 43 Features

Pour capturer toute la complexité cinématique, la structure topologique brute, les propriétés spectrales dynamiques, la physique capacitive du réseau et les particularités comportementales régionales, nous avons conçu un vecteur de caractéristiques ($\mathcal{X}$) composé de **43 descripteurs**. 

La météo (pluie, neige) a été exclue de ce vecteur d'apprentissage. En effet, inclure les conditions climatiques comme descripteurs directs dans le modèle XGBoost introduisait des instabilités et des biais de prédiction. Nous avons opté pour une modélisation découplée : la météo est appliquée comme un coefficient multiplicatif analytique sur la vitesse prédite sur sol sec, ou modélisée par des bases de données distinctes (scénarios de pluie et de neige purs).
...
### Importance des variables (Feature Importance CO2) et Performances

Le métamodèle en cascade a été entraîné sur la base consolidée des 19 simulations d'échelle réaliste (comprenant les nouvelles morphologies urbaines de Casablanca, Chamonix, Genève, Londres, Rio de Janeiro et Sydney). Les performances obtenues en validation croisée pour la prédiction du $CO_2$ sont résumées ci-dessous :

| Modèle | RMSE ($CO_2$ en kg) | MAE ($CO_2$ en kg) | Score $R^2$ (%) |
| :--- | :---: | :---: | :---: |
| **XGBoost (Cascade)** | **15 506,21** | **13 343,48** | **95,27 %** |
...
*   **XGBoost (Cascade)** : Il surclasse largement les autres approches avec un $R^2$ de $95,27\ %$. L'enchaînement à deux niveaux permet de contraindre les prédictions d'émissions par la réalité physique de la vitesse, évitant les aberrations prédictives sous forte charge de trafic.
...
#### Classement des variables les plus importantes (Feature Importance CO2)
L'extraction de l'importance relative des descripteurs dans le modèle XGBoost de prédiction du $CO_2$ (voir Figure D.7, Annexe D) met en lumière la hiérarchie d'influence physique suivante :
1.  **`nb_total_veh` (46,50 %)** : Le volume global de circulation reste le facteur de premier ordre.
2.  **`nb_voitures` (35,32 %)** : Le nombre de voitures détermine l'effort thermique de base.
3.  **`nb_motos` (8,25 %)** : La part de deux-roues motorisés, caractéristique des environnements asiatiques.
4.  **`duree_sim_s` (6,57 %)** : La durée totale d'exposition du réseau à la charge de trafic.
5.  **`nb_camions` (0,96 %)** : Les camions ont des taux d'émissions unitaires élevés.
6.  **`origin_Europe` (0,69 %)** : La distinction géographique (les villes européennes radiales réagissant différemment aux charges que les grilles rectilignes).
7.  **`pct_bus_ev` (0,42 %)** : L'électrification des bus est une variable d'action majeure.
8.  **`sigma_max_weighted` (0,38 %)** : La valeur singulière maximale pondérée (physique) s'impose comme la première métrique spectrale prédictive des surémissions.
9.  **`h2_norm_weighted` (0,24 %)** : La capacité d'accumulation de la congestion du réseau routier.
...
### Annexe A : Tableau des indicateurs macroscopiques des simulations complètes
...
### Annexe B : Tonnage de $CO_2$ émis par classe de véhicule de la flotte
...
### Annexe C : Profil de performance informatique de l'exécution (secondes CPU)
...
### Annexe D : Index des illustrations graphiques et des visualisations de simulation
...
#### Annexe D.1 : Planches relatives au hub de recharge de Sao Bien (Hanoï)
...
#### Annexe D.2 : Planches de topologie spectrale et mathématique des réseaux urbains
...
#### Annexe D.3 : Planches d'évaluation du modèle d'Intelligence Artificielle
...
#### Annexe D.4 : Planches de cartographie des congestions et heatmaps de simulation
